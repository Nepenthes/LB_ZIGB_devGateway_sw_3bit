...
sint8 sysTimeZone_H = 8; /*时区：时*///暂定东八区
				...
				if(true == weekend_judge(systemTime_current.time_Week, timDatsTemp_CalibrateTab[loop].Week_Num)){ //周占位对比
					...
					if(timCount_ENABLE == timDatsTemp_CalibrateTab[loop].if_Timing){ //定时开启标志核对
						...
						if((systemTime_current.time_Hour * 60 + systemTime_current.time_Minute) ==	\
						   (timDatsTemp_CalibrateTab[loop].Hour * 60 + timDatsTemp_CalibrateTab[loop].Minute) && //定时时刻核对，正确时刻前10s都是响应期
						...
						if((systemTime_current.time_Hour * 60 + systemTime_current.time_Minute) > //定时时刻核对，大于正确时刻标志清除

...
xQueueHandle xMsgQ_uartToutDats_datsComingCtrlEach;  //串口数据接收超时断帧数据队列-互控数据接收
...
bool  scenarioCtrlOpreatStart_FLG = false; //场景操作开始标志

bool  zigbNodeDevDetectManage_runningFLG = false; //设备链表监管运行标志
...
appMsgQueueCreat_uartToutDatsRcv(void){
	...
	else xMsgQ_uartToutDats_dataRemoteComing = xQueueCreate(40, sizeof(sttUartRcv_rmoteDatComming));
	...
	else xMsgQ_uartToutDats_datsComingCtrlEach = xQueueCreate(30, sizeof(sttUartRcv_ctrlEachDat));
	if(0 == xMsgQ_uartToutDats_datsComingCtrlEach)return FAIL;
	else return OK;
}
    ...
    while(p != NULL)    
    {
        ...
        while(q->next != NULL) 
        {
            if(q->next->nwkAddr == p->nwkAddr || (!memcmp(q->next->macAddr, p->macAddr, DEVMAC_LEN) && (q->next->devType == p->devType))) 
            {
				...
				memcpy(q->macAddr, r->macAddr, DEVMAC_LEN * sizeof(u8)); //后到优先更新
				q->nwkAddr = r->nwkAddr;
...
ZigBdevListInfoSaveToFlash(nwkStateAttr_Zigb *pHead){
	...
	stt_usrDats_zigbDevListInfo datsSave_Temp = {0};

	if(NULL == pHead){ //空表判断

		datsSave_Temp.zigbDevList_currentNum = 0;
		devParam_SlaveZigbDevListInfoSave(datsSave_Temp);
	...
	while(Disp->next != NULL){
		...
		memcpy(datsSave_Temp.listInfo[loop].macAddr, Disp->macAddr, DEVMAC_LEN);
		datsSave_Temp.listInfo[loop].nwkAddr = Disp->nwkAddr;
		loop ++;

		if(loop >= ZIGBDEV_LIST_LENGTH)break; //防溢出
	}

	datsSave_Temp.zigbDevList_currentNum = loop;
	devParam_SlaveZigbDevListInfoSave(datsSave_Temp);
	vTaskDelay(1);
}
...
ZigBdevListInfoRealesFromFlash(stt_usrDats_zigbDevListInfo *devListParam, nwkStateAttr_Zigb *pHead){
	...
	nwkStateAttr_Zigb *pAbove = pHead;
	nwkStateAttr_Zigb *pNew_temp;
	
	u8 loop = 0;

	if(pHead->next != NULL){

		os_printf("target listHead is not empty!.\n");
		return;
	
	}
	else
	if(devListParam->zigbDevList_currentNum){

		u8 loop = 0;

		for(loop = 0; loop < devListParam->zigbDevList_currentNum; loop ++){

			pNew_temp = (nwkStateAttr_Zigb *)os_zalloc(sizeof(nwkStateAttr_Zigb));
			
			memcpy(pNew_temp->macAddr, devListParam->listInfo[loop].macAddr, DEVMAC_LEN);
			pNew_temp->nwkAddr = devListParam->listInfo[loop].nwkAddr;
			pNew_temp->devType = DEVZIGB_DEFAULT;
			pNew_temp->onlineDectect_LCount = 60;
			pNew_temp->next = NULL;
...
//				uartTX_char(UART0, READ_PERI_REG(UART_FIFO(UART0)) & 0xFF);
			...
			{ //按条件填装队列
				...
				while(frameNum_reserve){
					...
					sttUartRcv_ctrlEachDat mptr_ctrlEachDat;
					...
					if((uart0_rxTemp[frameHead_insert] == ZIGB_FRAME_HEAD) && (frameNum_reserve >= frameTotal_Len)){ //超时断帧多收包只要断帧帧头为FE就没事，可做多重解析，重要的是帧头不是FE就有事
						...
						if(!memcmp(&uart0_rxTemp[frameHead_insert + 2], cmd_remoteDataComing, 2) || !memcmp(&uart0_rxTemp[frameHead_insert + 2], cmd_remoteNodeOnline, 2)){ //收到远端数据填装
							...
							if((uart0_rxTemp[frameHead_insert + 10] > CTRLEATHER_PORT_NUMSTART) && (frameTotal_Len == 27)){ //大于16为互控端口，总数据长度为27

								mptr_ctrlEachDat.prot_Ep = uart0_rxTemp[frameHead_insert + 10]; //终端点编号
								mptr_ctrlEachDat.ctrlEachOther_dat = uart0_rxTemp[frameHead_insert + 21]; //互控数据
								mptr_ctrlEachDat.ctrlEachOther_loop = uart0_rxTemp[frameHead_insert + 22]; //互控剩余发送次数
...
#define zigbRX_datsTransLen_ASR (8 + 25)
	uint8 buf_datsRX[zigbRX_datsTransLen_ASR] = {0};
...
#define zigB_datsTX_bufferLen 		128	//zigB数据发送缓存
	...
	u8 	local_scenarioRecord_dtRound = 0;  //静态值：当前为第几轮次记录
	u16 local_scenarioCtrl_counterRoundPause = 0; //数据发送轮次间距暂歇时间计数值
	u8 	local_ctrlRecord_reserveLoopInsert = zigB_ScenarioCtrlDataTransASY_opreatOnceNum; //静态值：场景可用操作单元数目(分批次专用)

	stt_threadDatsPass mptr_Z2S;	//通信线程发送消息队列缓存，zigbee通信主线程到socket通信主线程
	stt_threadDatsPass rptr_S2Z;	//通信线程接收消息队列缓存，socket通信主线程到zigbee通信主线程
	...
	const u16 detectPeriod_zigbSlaveDevListLen = 200; //zib子设备链表长度检测周期 单位：约10ms
	u16 detectCounter_zigbSlaveDevListLen = 0; //zib子设备链表长度检测计时计数缓存

	zigbDevList_Head->next = NULL; //链表复位

	vTaskDelay(100); //线程启动延时
	...
	stt_usrDats_zigbDevListInfo *datsRead_Temp = devParam_SlaveZigbDevListInfoRead();
	if(datsRead_Temp){

		if(datsRead_Temp->zigbDevList_currentNum > 0 && datsRead_Temp->zigbDevList_currentNum < 0xff){

			ZigBdevListInfoRealesFromFlash(datsRead_Temp, zigbDevList_Head);
			zigbNwkReserveNodeNum_currentValue = datsRead_Temp->zigbDevList_currentNum; //在线节点设备数量值更新
			os_printf("zigbee slaveDevList Reales from flash, num:%d.\n", datsRead_Temp->zigbDevList_currentNum);
		}
...
//						while(false == zigbDev_eptRemove_BYnwk((nwkStateAttr_Zigb *)pHead_listDevInfo,p_nwkRemove));
						...
						(zigbNwkReserveNodeNum_currentValue)?(ZigBdevListInfoSaveToFlash(zigbDevList_Head)):(ZigBdevListInfoSaveToFlash(NULL)); //设备链表信息flalsh保存
					}
				}
			}
		}
...
//										  local_datsRX->datsSTT.stt_MSG.Addr_from);
								...
								if(NULL == ZigbDevNew_temp){	//判断是否为新增节点设备，是则更新生命周期，否则添加进链表
									...
									if(memcmp(devMAC_Temp, ZigbDevNew_temp->macAddr, DEVMAC_LEN)){ //若网络地址相同的情况下，MAC地址不一致，则以最新的MAC地址为准，并进行更新

										os_printf(">>>detect differ MAC, node[%04X]Info get reales.\n", ZigbDevNew_temp->nwkAddr);
										memcpy(ZigbDevNew_temp->macAddr, devMAC_Temp, DEVMAC_LEN); //以最新的MAC地址进行对应链表节点更新
									}
...
//										localDataRecord_eaCtrl[0] = local_datsRX->datsSTT.stt_MSG.dats[0]; //本地记录
...
//										localDataRecord_eaCtrl[1] = local_datsRX->datsSTT.stt_MSG.dats[1]; //本地记录
...
//										localDataRecord_eaCtrl[2] = local_datsRX->datsSTT.stt_MSG.dats[1]; //本地记录
...
//								devStatus_pushIF = true; //开关状态数据推送
		...
		{ //互控业务
			...
			const u16 DTREQ_EXATTR_ONCEPERIOD = 251; //持续传输 数据包间隔
			u16 constandLoop_reserve = datsSend_requestEx[0].constant_Loop + datsSend_requestEx[1].constant_Loop + datsSend_requestEx[2].constant_Loop;
			
			if(constandLoop_reserve){
			
				if(!dtReqEx_counter){
				
					u16 current_Insert = constandLoop_reserve % 3;

					if((constandLoop_reserve % 3) == 0){

						os_printf(">>>dtCtrlEach loopParam:%d %d %d.\n", (int)datsSend_requestEx[0].constant_Loop,
																		 (int)datsSend_requestEx[1].constant_Loop,
																		 (int)datsSend_requestEx[2].constant_Loop);
					}
					
					while(!datsSend_requestEx[current_Insert].constant_Loop){
					...
					datsSend_requestEx[current_Insert].dats[1] = datsSend_requestEx[current_Insert].constant_Loop;
					ZigB_remoteDatsSend_straightforward(datsSend_requestEx[current_Insert].nwkAddr, datsSend_requestEx[current_Insert].portPoint, datsSend_requestEx[current_Insert].dats, datsSend_requestEx[current_Insert].datsLen);
					vTaskDelay(4); //40ms 协调性保底延时
...
#if(zigB_ScenarioCtrlDataTransASY_txBatchs_EN == 1) //宏控[是]：是否进行小批次分发
				do{
...
//									os_printf(">>>addrShud: %04X, addrQ:%04X.\n", localZigbASYDT_bufQueueScenarioReq[loop_Insert_temp].dataRespNwkAddr, rptr_scenarioCtrlResp.respNwkAddr);
				...
				if(local_insertRecord_datsReqScenario < zigB_ScenarioCtrlDataTransASY_opreatOnceNum){ //异步非阻塞场景控制数据请求（单轮内）
...
//										if(!(localZigbASYDT_scenarioCtrlReserveAllnum % zigB_ScenarioCtrlDataTransASY_opreatOnceNum))os_printf(">>>scenarioCtrl unitReserve:%d.\n", localZigbASYDT_scenarioCtrlReserveAllnum);
										if(!localZigbASYDT_scenarioCtrlReserveAllnum)os_printf(">>>scenarioCtrl allComplete.\n");
...
//										os_printf(">>>addrShud: %04X, addrQ:%04X.\n", localZigbASYDT_bufQueueScenarioReq[loop_Insert_temp].dataRespNwkAddr, rptr_scenarioCtrlResp.respNwkAddr);
		...
		/*>>>>>>zigb非阻塞子设备链表长度定时检测更新<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
		if(detectCounter_zigbSlaveDevListLen < detectPeriod_zigbSlaveDevListLen)detectCounter_zigbSlaveDevListLen ++;
		else{

			nwkStateAttr_Zigb *pHead_listDevInfo = (nwkStateAttr_Zigb *)zigbDevList_Head;

			detectCounter_zigbSlaveDevListLen = 0;

			zigbNwkReserveNodeNum_currentValue = zigbDev_chatLenDectect(pHead_listDevInfo);
		}

		{ //互控响应业务

			portBASE_TYPE xMsgQ_rcvResult = pdFALSE;
			sttUartRcv_ctrlEachDat rptr_ctrlEachDat = {0};

			xMsgQ_rcvResult = xQueueReceive(xMsgQ_uartToutDats_datsComingCtrlEach, (void *)&rptr_ctrlEachDat, (portTickType) 0);
			if(xMsgQ_rcvResult == pdTRUE){

				u8 localActLoop = 0;
				u8 statusRelay_temp = status_actuatorRelay;

...
LOCAL xTaskHandle pxCreatedTask_upgrade;
...
void upgrade_task(void *pvParameters)
{
	...
	os_printf("upgrade task start!!!\n");

    flash_erased=FALSE;
    precv_buf = (char*)os_malloc(UPGRADE_DATA_SEG_LEN);
...
upgrade_check(struct upgrade_server_info *server)
{
	...
	if(pxCreatedTask_upgrade){

		vTaskDelete(pxCreatedTask_upgrade);
	
	}else{

		os_printf("\n pxCreatedTask_upgrade == NULL.\n");	
	}
...
system_upgrade_start_ssl(struct upgrade_server_info *server)
{
    ...
    if(NULL == pxCreatedTask_upgrade){
		
        ret = xTaskCreate(upgrade_ssl_task, "upgrade_task", 800, server, 5, pxCreatedTask_upgrade);//1024, 890 left
...
system_upgrade_start(struct upgrade_server_info *server)
{
    ...
    ret = xTaskCreate(upgrade_task, "upgrade_task", 256, (void *)server, 5, &pxCreatedTask_upgrade);//1024, 890 left

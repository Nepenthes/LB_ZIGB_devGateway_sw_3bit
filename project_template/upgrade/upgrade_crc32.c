...
upgrade_crc_check(uint16 fw_bin_sec ,unsigned int sumlength)
{
	...
	os_printf("img_crc = 0x%08X\n",img_crc);
	spi_flash_read(start_sec * SPI_FLASH_SEC_SIZE + sumlength - 4,&flash_crc, 4);
    os_printf("flash_crc = 0x%08X\n",flash_crc);

	//11000011100010000111100010101011
	//00111100011101111000011101010101
	//避免crc码以补码形式出现，进行补码逆向还原进行规避
	if(img_crc == flash_crc) {
	    return 0;
	} else {

		if(img_crc & 0x80000000){	//补偿逻辑，crc重新矫正

			u32 crcTemp = ~(img_crc - 1);	//补码逆过程
			
			os_printf("img_crc<correction>: 0x%08X\n", crcTemp);
			if(crcTemp == flash_crc){
				return 0;
			}else{
				return -1;
			}

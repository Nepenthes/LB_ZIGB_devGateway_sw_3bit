...
LOCAL void normalBussiness_shortTouchTrig(u8 statusPad, bool shortPressCnt_IF){
	...
	if(!shortPressCnt_IF){ //非连按才触发互控
		...
		if(SWITCH_TYPE == SWITCH_TYPE_SWBIT1 || SWITCH_TYPE == SWITCH_TYPE_SWBIT2 || SWITCH_TYPE == SWITCH_TYPE_SWBIT3)EACHCTRL_realesFLG = swCommand_fromUsr.objRelay; //有效互控触发(直接触发对应键互控)
		else
...
touchPad_functionTrigContinue(u8 statusPad, u8 loopCount){	//普通触摸按键连按触发
	...
	if(SWITCH_TYPE == SWITCH_TYPE_SWBIT1 || SWITCH_TYPE == SWITCH_TYPE_SWBIT2 || SWITCH_TYPE == SWITCH_TYPE_SWBIT3)EACHCTRL_realesFLG = swCommand_fromUsr.objRelay; //连按最后一次触发有效互控
	else
	if(SWITCH_TYPE == SWITCH_TYPE_CURTAIN)EACHCTRL_realesFLG = 1; //有效互控触发

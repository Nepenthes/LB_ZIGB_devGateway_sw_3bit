...
bool combinationFunTrigger_3S1L_standBy_FLG = false;  //三短一长预触发标志
bool combinationFunTrigger_3S5S_standBy_FLG = false;  //三短五短预触发标志
...
touchPad_functionTrigNormal(u8 statusPad, keyCfrm_Type statusCfm){ //普通触摸按键触发
	...
	switch(statusCfm){
		...
		case press_ShortCnt:{
			...
			if(touchKeepCnt_record == 3){
				...
				combinationFunTrigger_3S1L_standBy_FLG = true; //特殊组合动作预触发标志置位<3短1长>
		...
		case press_LongA:{
			...
			if(combinationFunTrigger_3S1L_standBy_FLG){ //特殊组合按键动作业务触发<3短1长>
				...
				combinationFunTrigger_3S1L_standBy_FLG = false; //预触发标志清零
	...
	{ //按键特殊组合动作相关标志及变量清零
		...
		if(statusCfm != press_ShortCnt){
			...
			combinationFunTrigger_3S1L_standBy_FLG = false; //特殊组合动作预触发标志复位<3短1长>
			
			if(statusCfm != press_Short)combinationFunTrigger_3S5S_standBy_FLG = false; //非短按及非连续短按，预触发标志复位<3短5短>
		}
	}
}
...
touchPad_functionTrigContinue(u8 statusPad, u8 loopCount){	//普通触摸按键连按触发
	...
	switch(loopCount){
		...
		case 3:{
			...
			combinationFunTrigger_3S5S_standBy_FLG = true; //特殊组合动作按键预触发标志置位<3短5短>
		...
		case 5:{
			...
			if(combinationFunTrigger_3S5S_standBy_FLG){ //特殊组合按键动作业务触发<3短5短>
	...
	{ //按键特殊组合动作相关标志及变量清零
		...
		if(loopCount != 3){ //
			...
			combinationFunTrigger_3S5S_standBy_FLG = false; //特殊组合动作预触发标志复位<3短5长>
		}
	}
}
...
touchPad_Scan(void){
	...
	if(!combinationFunFLG_3S5S_cancel_counter)combinationFunTrigger_3S5S_standBy_FLG = false; //<3短5短>特殊组合按键衔接时间超时检测业务，超时则将对应预触发标志复位
